...
export function useSmartFetch<T = any>(initialOptions?: FetchOptions) {
  ...
  const validateField = useCallback(async (fieldType: 'email' | 'username', value: string) => {
    ...
    // Mock validation for testing - replace with real API call later
    await new Promise(resolve => setTimeout(resolve, 300)); // Simulate network delay
    
    // Mock some taken usernames/emails for testing
    const takenUsernames = ['admin', 'test', 'user', 'demo'];
    const takenEmails = ['test@example.com', 'admin@example.com', 'demo@example.com'];
    
    if (fieldType === 'username' && takenUsernames.includes(value.toLowerCase())) {
      return 'Username already exists';
    }
    
    if (fieldType === 'email' && takenEmails.includes(value.toLowerCase())) {
      return 'Email already exists';
    }
    
    return null; // Available
    
    // Real API validation (commented out to avoid JSON parse error)
    // try {
